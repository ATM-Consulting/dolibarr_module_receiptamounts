...
/**
 * \file    test/phpunit/ReceiptAmountsFunctionalTest.php
...
/**
 * Class ReceiptAmountsFunctionalTest
 *
 ...
 *
 * @package Testreceiptamounts
 */
class ReceiptAmountsFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modReceiptAmounts')->click();
		} else {
			// Disable the module
			$this->byHref('modReceiptAmounts')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/receiptamounts/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('receiptamounts/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/receiptamounts/admin/about.php');
		$this->authenticate();
		return $this->assertContains('receiptamounts/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('receiptamountswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modReceiptAmounts_ReceiptAmountsTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modReceiptAmounts_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
