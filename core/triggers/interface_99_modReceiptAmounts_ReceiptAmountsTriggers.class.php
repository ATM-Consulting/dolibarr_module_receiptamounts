...
/**
 * \file    core/triggers/interface_99_modReceiptAmounts_ReceiptAmountsTriggers.class.php
 ...
 *      - interface_99_modReceiptAmounts_MyTrigger.class.php
...
/**
 *  Class of triggers for ReceiptAmounts module
 */
class InterfaceReceiptAmountsTriggers extends DolibarrTriggers
{
	/**
	 * Constructor
	 *
	...
	public function __construct($db)
	{
		...
		$this->family = "demo";
		$this->description = "ReceiptAmounts triggers.";
	...
	{
		if (empty($conf->receiptamounts) || empty($conf->receiptamounts->enabled)) {
	...
	public function linereceptionUpdate($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		if (!empty($object->fk_reception))
		{
			...
			$receipt = new Reception($object->db);
			if($receipt->fetch($object->fk_reception) > 0)
			{
	...
	public function linereceptionDelete($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		if ($res > 0 && !empty($object->fk_reception))
		{
			require_once DOL_DOCUMENT_ROOT.'/reception/class/reception.class.php';

			$receipt = new Reception($object->db);
			$res = $receipt->fetch($object->fk_reception);

			if($res > 0)
			{
				if (!empty($receipt->lines))
				{
					// delete line for lines array before calculate
					foreach ($receipt->lines as $k =>$line)
					{
						if ($line->id == $object->id)
						{
							unset($receipt->lines[$k]);
							break;
						}
					}
				}
